...
#	       300,100);
    ...
    my $pledge_text = $self->{'pledge'}{'text'};
...
R,80,
XY,$page_centre,120
B,20,Go to pledgebank.com/$self->{'pledge'}{'name'}
...
OUTPUT
    ...
    my $text = $page->text;
    $text->compress; # CPU cheap, bandwidth not so cheap
...
#    $text->translate($x,$y);
	...
	if ($self->{'pledge'}{'truetype-font'} =~ /(.+).ttf/) {
...
MS::PDF - create PDF posters and flyers for PledgeBank.

=head1 SYNOPSIS

my $pdf = MS::PDF->new( { 'pledge-id' => 0,
			  'target' => 100,
			  'name' => 'whales',
